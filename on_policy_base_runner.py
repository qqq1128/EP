...
class OnPolicyBaseRunner:
    ...
    def __init__(self, args, algo_args, env_args):
        ...
        if self.algo_args["render"]["use_render"]:  # make envs for rendering
            ...
            self.envs = make_train_env(
                ...
                env_args,
                self.args["algo"]  ## 自己新加的变量传参 ，可以让gym识别到algorithm
    ...
    def run(self):
        ...
        for episode in range(1, episodes + 1):
            ...
            self.rewards_log.append(0)

            #print(self.algo_args["train"]["episode_length"])

            for step in range(self.algo_args["train"]["episode_length"]):

                print("\rtraining step:{}:{}:{}".format(step+1,episode, episodes + 1), end=" ")
                ...
                # rewards_log[episode-1] += rewards[0]
                ...
                if (dones[0][0]):
                    # 结束后新建一个reward.append
                    self.rewards_log[-1] += rewards[0][0]
                    print("\n")
                    #self.rewards_log 尬，上面已经有了reward，这里又加了一次
                else:
                    # 否则在原来的reward累加
                    self.rewards_log[-1] += rewards[0][0]
        ...
        # 绘制 self.eval_episode_rewards 曲线
        # plt.figure()
        ...
        # 将 self.eval_episode_rewards 保存mat
        print("save train_episode_rewards"+self.args["algo"]+".mat")
        sio.savemat('train_episode_rewards_'+self.args["algo"]+'.mat', {'train_episode_rewards': self.rewards_log})
    ...
    def eval(self):
        """Evaluate the model."""
        self.logger.episode_init(self.episode)  # logger callback at the beginning of evaluation
